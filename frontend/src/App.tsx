...
import { shortenUrl, login, register, getMyUrls, deleteUrl, type ShortenResponse, type UrlHistoryItem } from './api';
    ...
    <>
      ...
      <main>
        {!user ? (
          ...
          <>
            {/* ─── Dashboard: Shortener Bar ─── */}
            <section className="hero-section" style={{ textAlign: 'center', overflow: 'hidden' }}>
              <div className="hero-bg" /><div className="hero-grid" />
              <div className="container" style={{ position: 'relative', zIndex: 1 }}>
                <div style={{ animation: 'fadeInUp 0.5s ease-out' }}>
                  <h1 style={{ fontSize: 'clamp(32px, 5vw, 48px)', fontWeight: 900, letterSpacing: '-1px', lineHeight: 1.1, marginBottom: 16 }}>
                    Shorten links,{' '}
                    <span className="gradient-text">instantly</span>
                  </h1>
                  <p style={{ color: 'var(--text-secondary)', fontSize: 16, maxWidth: 480, margin: '0 auto 36px', lineHeight: 1.6 }}>
                    Custom aliases, QR codes, password protection, and expiration dates — all in one place.
                  </p>
                </div>
                <div style={{ animation: 'fadeInUp 0.5s ease-out 0.1s', animationFillMode: 'both' }}>
                  <UrlShortenerBar onSuccess={() => { announce('URL shortened successfully'); setUrlHistoryKey(k => k + 1); }} />
                </div>
              </div>
            </section>
...
function UrlHistory() {
  ...
  useEffect(() => { const cancel = loadUrls(); return cancel; }, [loadUrls]);
  ...
  const handleDelete = async (alias: string) => {
    try {
      ...
      setQrCache(prev => { const next = { ...prev }; delete next[alias]; return next; });
    } catch (err) {
      // silently fail, could add toast later
    }
  };

  const handleToggleQr = async (alias: string, shortUrl: string) => {
    if (expandedQr === alias) {
      setExpandedQr(null);
      return;
    }
    setExpandedQr(alias);
    if (!qrCache[alias]) {
      QRCode.toDataURL(shortUrl, { width: 180, margin: 2, color: { dark: '#0d0d16', light: '#ffffff' } })
        .then(dataUrl => setQrCache(prev => ({ ...prev, [alias]: dataUrl })))
        .catch(() => {});
    }
  };
          ...
          {urls.map((u, i) => (
            <div key={u.alias} className="history-item" style={{ animation: 'fadeInUp 0.4s ease-out', animationFillMode: 'both', animationDelay: `${i * 0.05}s` }}>
              ...
              <div className="history-item-actions">
                <button onClick={() => handleToggleQr(u.alias, u.short_url)} className="btn-ghost" style={{ padding: '6px 14px', fontSize: 12 }}>
                  {expandedQr === u.alias ? 'Hide QR' : 'QR'}
                </button>
                ...
                <button onClick={() => handleDelete(u.alias)} className="btn-ghost btn-delete" style={{ padding: '6px 14px', fontSize: 12 }}>
                  <svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                </button>
              </div>
            </div>
          ))}
